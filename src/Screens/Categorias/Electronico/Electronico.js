...
export default function Electronico() {
    ...
    const navigateToAddProduct = () => {
        navigation.navigate('AddProductoElectro');
    };

    const navigateToDetails = (product) => {
        navigation.navigate('DetallesProductoElectro', { product });
    };
    const fetchItems = async () => {
        const itemsCollection = [];
        const q = query(collection(db, 'productos', 'electronicos', 'items'));
    ...
    const deleteProduct = async (productId) => {
        const productRef = doc(db, 'productos', 'electronicos', 'items', productId);
        try {
